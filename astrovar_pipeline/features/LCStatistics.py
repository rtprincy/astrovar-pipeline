...
class LCStatistics:
    ...
    def abbe_value(self):
        ...
        abbe = sum((ysorted[1:] - ysorted[:-1]) ** 2) / (
            2 * sum((ysorted - np.mean(ysorted)) ** 2)
        )
        return abbe
    ...
    def amplitude(self):
        if self.Time is None or self.best_freq is None:
            raise ValueError(
                "Time and best_freq are required for amplitude computation."
            )
    ...
    def log_sigvar(self):
        if self.flux is None or self.flux_err is None:
            raise ValueError(
                "Flux and flux_err are required for log_sigvar computation."
            )
        w = 1 / (self.flux_err**2)
        n = self.flux.size
        w_avg = np.mean(w)
        if n > 1:
            neta = (n / (n - 1)) * (
                np.mean(w * (self.flux**2)) - (np.mean(w * self.flux) ** 2) / w_avg
            )
        else:
            neta = np.nan
    ...
    def psi_sigvar(self):
        if self.frequencies is None or self.psi is None:
            raise ValueError(
                "frequencies and psi are required for psi_sigvar computation."
            )

        mad = stats.median_abs_deviation(
            np.array([[self.frequencies, self.psi_norm]]), axis=None
        )
        ...
        if n > 1:
            neta = (n / (n - 1)) * (
                np.mean(w * (self.psi_norm**2))
                - (np.mean(w * self.psi_norm) ** 2) / w_avg
            )
        else:
            neta = np.nan
    ...
    def fap(self):
        ...
        lsp_standard = utils.convert_normalization(
            Z=self.lspower,
            N=len(self.mag),
        ...
        fap = lsp.false_alarm_probability(
            lsp_standard.max(),
            samples_per_peak=10,
            minimum_frequency=self.frequencies.min(),
            maximum_frequency=self.frequencies.max(),
        )
