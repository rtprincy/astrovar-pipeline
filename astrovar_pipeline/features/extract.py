...
def extract_features_from_lc(oversampling_factor: np.int32, lc: pd.DataFrame, per_g: PeriodogramBundle|None=None,per_bp: PeriodogramBundle|None=None,per_rp: PeriodogramBundle|None=None) -> Dict[str, Any]:
    feats = {}
    ...
    if (per_g is not None) & (per_bp is not None) & (per_rp is not None):
        ...
        best_freq_g,best_period_g = optimise_freq(oversampling_factor,per_g.psi,per_g.freq,Time_g,mag_g,mag_err_g)
        best_freq_bp,best_period_bp = optimise_freq(oversampling_factor,per_bp.psi,per_bp.freq,Time_bp,mag_bp,mag_err_bp)
        best_freq_rp,best_period_rp = optimise_freq(oversampling_factor,per_rp.psi,per_rp.freq,Time_rp,mag_rp,mag_err_rp)
