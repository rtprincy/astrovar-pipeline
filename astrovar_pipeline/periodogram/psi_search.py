...
def psi_periodogram(time, mag, err, min_freq, max_freq, oversample=5) -> Dict[str, np.ndarray]:
    """Compute Ψ-statistic = 2*LSP / theta, combining LS and LK.
    Returns dict with 'freq', 'lsp', 'theta', 'psi'.
    """

    frequencies = freq_grid(time, oversample_factor=oversample, f0=min_freq, fn=max_freq)
